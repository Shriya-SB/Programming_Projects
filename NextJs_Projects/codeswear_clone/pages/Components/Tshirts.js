...
const Tshirts = ({ darkMode, products }) => {
    ...
    return (
        <>
            <section className={`min-h-screen body-font ${darkMode ? "dark:bg-gray-700 dark:text-white" : ""}`}>
                <h1 className='text-4xl text-center py-5 font-bold'>TShirts for sale.</h1>
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-wrap -m-4">
                        {Object.keys(products).length === 0 && <h4 className='text-center justify-center items-center mx-auto text-3xl font-bold'>Sorry! TShirts are out of stock. New stock coming soon..</h4>}
                        {Object.keys(products).map((item) => {
                            return <>
                                <Link key={products[item]._id} href={`/product/${products[item].slug}`} className='lg:w-1/4 mx-auto md:w-full p-4 w-full'>
                                    <div className="block relative shadow-xl rounded overflow-hidden">
